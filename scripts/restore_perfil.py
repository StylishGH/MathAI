...
# Get the original valid version
old_content = subprocess.check_output(["git", "show", "2e380d8:src/app/pages/perfil.py"]).decode("utf-8")

# Apply change 1: Remove "Física 1" from MATERIAS_SUPERIOR
old_content = old_content.replace('"Física 1", ', '')

# Apply change 2: Consentimento UI
...
old_content = old_content.replace("    c_save_esq, c_save_btn, c_save_dir = st.columns([1, 2, 1])", consent_ui)

# Apply change 3: Consentimento SQL
...
old_content = old_content.replace(old_sql, new_sql)

# Apply change 4: Consentimento Session
