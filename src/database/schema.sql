...
-- Fase 0 (V0) - Fundação de Dados com suporte a Repetição Espaçada (SM-2)
-- Fase 2 (V2) - Dataset de Treino para IA Própria
-- ====================================================================
...
-- 2. Tabela de Conceitos & Gatilhos Cognitivos (Estilo Flashcards / Anki)
...
-- 3. Tabela de Tentativas (Histórico de Resolução)
-- Registra cada sessão de estudo: tempo, estratégia adotada, erro e metacognição.
CREATE TABLE IF NOT EXISTS tentativas (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    questao_id INTEGER NOT NULL,
    aluno_id TEXT NOT NULL DEFAULT 'default',  -- Identificador do aluno (para multi-usuário)
    ...
    tipo_erro TEXT DEFAULT 'nenhum' CHECK (
        tipo_erro IN ('nenhum', 'conta_sinal', 'manipulacao_algebrica', 'conceitual', 'interpretacao', 'outro')
    ),
...
-- 4. Tabela de Repetição Espaçada (Spaced Repetition / Algoritmo SM-2)
-- Controla quando uma questão ou conceito deve ser revisado novamente.
CREATE TABLE IF NOT EXISTS revisao_espacada (
    ...
    UNIQUE (item_tipo, item_id)                -- Garante apenas 1 registro de agendamento por item
);

-- 5. Tabela de Perfil do Aluno por Tópico (Métricas Agregadas)
-- Serve como "cache" de performance para carregar instantaneamente o Dashboard no Streamlit.
...
-- 6. Tabela de Diagnósticos de IA (Visão Multimodal & OCR)
-- Registra as transcrições em LaTeX e avaliações pedagógicas geradas pelo Gemini.
-- Serve como dataset de fine-tuning/treinamento para o modelo próprio em PyTorch (Fase 6).
CREATE TABLE IF NOT EXISTS diagnosticos_ia (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    tentativa_id INTEGER,                      -- FK opcional para tentativas
    questao_id INTEGER NOT NULL,
    aluno_id TEXT NOT NULL DEFAULT 'default',  -- Identificador do aluno
...
-- 7. Tabela de Log de Dicas Socráticas (Dataset de Comportamento de Estudo)
-- Registra cada vez que um aluno pediu ajuda, em qual nível e o que o Gemini respondeu.
-- Revela o padrão de dependência de dicas — dado valioso para o modelo próprio.
...
-- 8. Tabela de Usuários (Login, Faturamento & Onboarding)
-- Armazena o cadastro dos alunos, dados de cobrança (CPF, CEP, Endereço) e status de verificação.
CREATE TABLE IF NOT EXISTS usuarios (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    nome TEXT NOT NULL,
    email TEXT NOT NULL UNIQUE,
    senha_hash TEXT NOT NULL,              -- SHA-256 da senha (sem dependências externas)
...
-- 9. Tabela de Códigos de Verificação (2FA / OTP)
...
CREATE INDEX IF NOT EXISTS idx_revisao_proxima ON revisao_espacada (proxima_revisao);
