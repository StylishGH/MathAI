...
def importar_lote_mathnet(qtd=5, apenas_com_figura=True):
    ...
    for i, raw_row in enumerate(ds):
        ...
        enunciado_original = str(row.get("problem_markdown", "")).strip()
        ...
        if tem_imagem:
            ...
            if hasattr(img_item, "save"):
                img_item.save(caminho_img_disco)
            elif isinstance(img_item, dict) and "bytes" in img_item and img_item["bytes"]:
                caminho_img_disco.write_bytes(img_item["bytes"])
        ...
        ano_int = int(str(ano_val)) if ano_val is not None and str(ano_val).isdigit() else None
